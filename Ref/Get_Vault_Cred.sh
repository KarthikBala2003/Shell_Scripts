...
# Set -x
# set -o posix
# sec_key="$(echo $vault_creds| grep -Po '"sec_usr_id": *\K"([^"]*")' | sed 's/"//g')" 

#sec_key="$(echo $vault_creds Brep -Po ($KEY_VAL_NEW): *\K" (["]*")*)" 

#set +o posix
#set +x
# sec_val="$ (echo $vault_creds | grep -oP "sec_usr_key"; *\K"([^"]*")" sed 's/"//g')"
# echo Secret Key: $sec key 
