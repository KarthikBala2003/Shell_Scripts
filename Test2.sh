...
# docker run -d --name $CUSTOMIMAGE_NAME -p $CUSTOM_PORT:$DOCKER_INTERNAL_PORT $BASEIMAGE_NAME
...
echo
docker exec -it $CUSTOMIMAGE_NAME bash

cd htdocs/

touch $HTML_PAGE_NAME
# cat > $HTML_PAGE_NAME 

echo $CUSTOM_MSG >> $HTML_PAGE_NAME
...
# EOF
# docker commit <container_id> | cut -d ':' -f2 | cut -c1-12
